...
| OpenCode | Generar vistas para reportes y aplicar criterios de seguridad (Parte B) | Vistas en `specs/02-vistas.md` | Generar `v_productos_vigentes`, `v_pedidos_usuario` y `v_usuarios_seguros` excluyendo la contraseña | Se aceptaron las tres vistas y se verificó la equivalencia de resultados con consultas directas usando `EXCEPT` |
